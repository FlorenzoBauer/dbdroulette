  ...
  useEffect(() => {
    ...
    if (window.electronAPI) {
      ...
      const updateHandlers = {
        ...
        'update-available': (event, releaseData) => {
          console.log('Update available in UpdateScreen:', releaseData);
          setStatus('available');
          // Auto-download after 2 seconds
          setTimeout(() => {
            if (window.electronAPI && window.electronAPI.downloadUpdate) {
              window.electronAPI.downloadUpdate(releaseData);
            }
          }, 2000);
        },
